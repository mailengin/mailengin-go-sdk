...
func (service *EmailsService) Send(ctx context.Context, request SendEmailRequest) (*SendEmailResponse, error) {
	if strings.TrimSpace(request.To) == "" {
		return nil, errors.New("mailengin.Emails.Send requires To")
	}
	if err := requireContent(request.TemplateName, request.TemplateID, request.HTML, request.Subject); err != nil {
		return nil, err
	}
	...
	if request.ReplyToMailEngin != nil {
		payload["reply_to_mailengin"] = *request.ReplyToMailEngin
	}
	var wire struct {
		ID           string  `json:"id"`
	...
	if err := service.client.post(ctx, "/api/developer/send", payload, &wire); err != nil {
		return nil, err
	}
...
func (service *EmailsService) SendBulk(ctx context.Context, request SendBulkEmailRequest) (*SendBulkEmailResponse, error) {
	if len(request.To) == 0 {
		return nil, errors.New("mailengin.Emails.SendBulk requires recipients")
	}
	if len(request.To) > 1000 {
		return nil, errors.New("mailengin.Emails.SendBulk accepts up to 1000 recipients")
	}
	if err := requireContent(request.TemplateName, request.TemplateID, request.HTML, request.Subject); err != nil {
		return nil, err
	}
	...
	for _, recipient := range request.To {
		if strings.TrimSpace(recipient.Email) == "" {
			return nil, errors.New("every bulk recipient requires an email address")
		}
		if recipient.Variables == nil {
			recipients = append(recipients, recipient.Email)
		} else {
			recipients = append(recipients, map[string]any{"email": recipient.Email, "variables": recipient.Variables})
		}
	}
	...
	if err := service.client.post(ctx, "/api/developer/send-bulk", payload, &wire); err != nil {
		return nil, err
	}
...
func requireContent(templateName, templateID, html, subject string) error {
	...
	if !hasTemplate && strings.TrimSpace(html) == "" {
		return errors.New("provide TemplateName, TemplateID, or HTML")
	}
	if !hasTemplate && strings.TrimSpace(subject) == "" {
		return errors.New("raw HTML sends require Subject")
	}
	return nil
}
