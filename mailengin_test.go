...
func TestSendMapsRequestAndResponse(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/developer/send" { t.Errorf("unexpected path: %s", r.URL.Path) }
		if got := r.Header.Get("Authorization"); got != "Bearer re_test_key" { t.Errorf("authorization = %q", got) }
		if got := r.Header.Get("User-Agent"); got != "mailengin-go/0.1.0" { t.Errorf("user agent = %q", got) }
		var body map[string]any
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil { t.Fatal(err) }
		if body["template_name"] != "welcome" { t.Errorf("template_name = %#v", body["template_name"]) }
	...
	if err != nil { t.Fatal(err) }
	if result.ID != "msg_1" { t.Fatalf("id = %q", result.ID) }
}
...
func TestRateLimitAndValidation(t *testing.T) {
	...
	apiErr, ok := err.(*Error)
	if !ok || apiErr.Status != 429 || apiErr.RetryAfter == nil || *apiErr.RetryAfter != 12 || !apiErr.IsRetryable() { t.Fatalf("unexpected error: %#v", err) }
...
func TestBulkMappingLimitAndCancellation(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/developer/send-bulk" { t.Errorf("unexpected path: %s", r.URL.Path) }
		var body map[string]any
		if err := json.NewDecoder(r.Body).Decode(&body); err != nil { t.Fatal(err) }
		if len(body["to"].([]any)) != 2 { t.Fatalf("unexpected recipients: %#v", body["to"]) }
	...
	if err != nil || result.JobID != "bulk_1" { t.Fatalf("unexpected result: %#v, %v", result, err) }
	tooMany := make([]BulkRecipient, 1001)
	for index := range tooMany { tooMany[index] = BulkRecipient{Email: "a@example.com"} }
	if _, err := client.Emails.SendBulk(context.Background(), SendBulkEmailRequest{To: tooMany, TemplateName: "welcome"}); err == nil { t.Fatal("expected recipient limit error") }

	canceled, cancel := context.WithCancel(context.Background())
	cancel()
	if _, err := client.Emails.Send(canceled, SendEmailRequest{To: "a@example.com", TemplateName: "welcome"}); err == nil { t.Fatal("expected cancellation error") }
}
